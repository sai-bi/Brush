void ColorSketch::tempbrush()
{
	...
	for (int i = 0; i < regions.size(); ++i)
	{
		...
		for(int j = 0;j < boundary_point_number;j++){
			...
			/*
			if(angle > 0){
				double temp = sqrt(1 + pow(tan(angle),2.0));
				tangent(j,0) = 1 / temp;
				tangent(j,1) = tan(angle) / temp;
			}
			else{
				double temp = sqrt(1 + pow(tan(angle),2.0)); 
				tangent(j,0) = 1 / temp;
...
Mat_<double> ColorSketch::place_brush(const Mat& input_image, const Mat_<double>& tangent,
