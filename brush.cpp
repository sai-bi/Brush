...
void smooth_tangent(Mat_<Point2d>& tangent, const Mat_<int> region_id,int current_region_id,
        const Mat_<double>& dist, const Mat_<int>& current_region_point){
    ...
    while(iter_num< 5){
        ...
        for(int i = 0;i < current_region_point.rows;i++){
            ...
            int point_y = current_region_point(i,1); 
            ...
            for(int p = -neighbour_num;p < neighbour_num + 1;p++){
                for(int k = -neighbour_num;k < neighbour_num+1;k++){
                    ...
                    double temp = tangent_angle(y,x) - tangent_angle(point_y,point_x);
                    count++;
            ...
            if(count == 0)
                continue;
            new_tangent_angle(point_y,point_x) += delta_theta/count;
        }
        iter_num++;
